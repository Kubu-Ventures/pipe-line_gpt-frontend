  ...
  return (
    <div style={{ fontFamily: F, background: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <header className="lp-nav" style={{
        height: 75, flexShrink: 0, background: '#fff',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <LogoIcon />
          <span style={{ fontWeight: 700, fontSize: 18, color: DARK, letterSpacing: '-0.02em' }}>PipelineGPT</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <LangSwitcher variant="light" />
          <Link href="/login" style={{
            fontSize: '1rem', fontWeight: 400, color: DARK, textDecoration: 'none',
            letterSpacing: '0.01em', display: 'flex', alignItems: 'center', gap: 4,
          }}>
            {tn('signIn')} <span style={{ color: BLUE }}>→</span>
          </Link>
        </div>
      </header>

      {/* HERO */}
      <section style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'flex-end', minHeight: 400 }}>
        ...
        <img
          ...
          loading="eager"
        />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(35,46,62,0.90) 0%, rgba(35,46,62,0.62) 50%, rgba(35,46,62,0.20) 100%)' }} />
        ...
        <div className="lp-hero-content" style={{ position: 'relative', zIndex: 10 }}>
          ...
          <p style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: AZURE, marginBottom: 20 }}>
            {t('heroEyebrow')}
          </p>
          <h1 style={{ fontWeight: 700, fontSize: 'clamp(1.75rem, 5vw, 3.5rem)', lineHeight: '110%', letterSpacing: '-0.025em', color: '#fff', marginBottom: 20, maxWidth: 720 }}>
            {t('heroHeadline')}
          </h1>
          <p style={{ fontSize: 'clamp(0.9375rem, 2vw, 1.125rem)', fontWeight: 400, lineHeight: '138%', color: 'rgba(255,255,255,0.68)', marginBottom: 40, maxWidth: 540 }}>
            {t('heroSubtext')}
          </p>
      ...
      {/* FEATURES */}
      <section className="lp-features" style={{ background: '#fff', borderTop: `4px solid ${YELLOW}` }}>
        {FEATURES.map((f, i) => (
          <div key={f.title} className="lp-feature-cell"
            style={{ borderRight: i < FEATURES.length - 1 ? `1px solid ${GRAY_300}` : 'none' }}>
            <div>{f.icon}</div>
            <p style={{ fontSize: '1rem', fontWeight: 700, color: TEXT, lineHeight: '110%', letterSpacing: '-0.01em' }}>{f.title}</p>
